...
const SocialLinks = () => {
  ...
  return (
    <div className='flex flex-col fixed top-[55%] md:top-[35%] left-0'>
      <ul>
        {links.map(({id,href,child,style,download})=>(
        <li key={id} className={`opacity-80 md:opacity-100 w-40 h-14 flex justify-between items-center px-4 ml-[-100px] hover:ml-[-10px] hover:rounded-md duration-300 bg-gray-500 ${style}`}>
